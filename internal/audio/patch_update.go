...
func (e *Engine) UpdatePatch(compiled *patchmodel.CompiledPatch, bpm float64, changed, removed []patchmodel.InstrumentID) (PatchUpdateResult, error) {
	...
	entry := PatchUpdateTrace{RequestID: requestID, RequestedFrame: frame, AppliedFrame: frame, OldGeneration: oldGeneration, NewGeneration: uint64(compiled.Generation), OldFingerprint: oldFingerprint, NewFingerprint: compiled.Fingerprint, InvalidatedHandles: invalidated, Result: "applied", Duration: time.Since(started)}
