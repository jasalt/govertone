...
type (
	...
	GoSynth struct {
		...
		instrumentControl map[int]float32
		voiceControl      [MAX_VOICES]map[int]float32
	}
...
func (s GoSynther) Synth(patch sointu.Patch, bpm int) (sointu.Synth, error) {
	...
	ret := &GoSynth{bytecode: *bytecode, stack: make([]float32, 0, 4), delaylines: make([]delayline, patch.NumDelayLines()), instrumentControl: map[int]float32{}}
...
// SetControl installs an absolute persistent parameter value for one voice.
func (s *GoSynth) SetControl(voiceIndex, operand int, value float32) error {
	if voiceIndex < 0 || voiceIndex >= MAX_VOICES {
		return fmt.Errorf("voice index %d out of range", voiceIndex)
	}
	if operand < 0 || operand >= len(s.bytecode.Operands) {
		return fmt.Errorf("control operand %d out of range", operand)
	}
	if math.IsNaN(float64(value)) || math.IsInf(float64(value), 0) {
		return errors.New("control value must be finite")
	}
	if s.voiceControl[voiceIndex] == nil {
		s.voiceControl[voiceIndex] = map[int]float32{}
	}
	s.voiceControl[voiceIndex][operand] = value
	return nil
}

// SetInstrumentControl installs an absolute persistent value for every voice
// executing the operand. Operand offsets are instrument-specific.
func (s *GoSynth) SetInstrumentControl(operand int, value float32) error {
	if operand < 0 || operand >= len(s.bytecode.Operands) {
		return fmt.Errorf("control operand %d out of range", operand)
	}
	if math.IsNaN(float64(value)) || math.IsInf(float64(value), 0) {
	...
	s.instrumentControl[operand] = value
	return nil
}

func (s *GoSynth) ClearControl(voiceIndex, operand int) {
	if voiceIndex >= 0 && voiceIndex < MAX_VOICES && s.voiceControl[voiceIndex] != nil {
		delete(s.voiceControl[voiceIndex], operand)
	}
}
func (s *GoSynth) ClearInstrumentControl(operand int) { delete(s.instrumentControl, operand) }
func (s *GoSynth) ClearVoiceControls(voiceIndex int) {
	if voiceIndex >= 0 && voiceIndex < MAX_VOICES {
		s.voiceControl[voiceIndex] = nil
	}
}
func (s *GoSynth) ParameterOperand(address ParameterAddress) (int, bool) {
	offset, ok := s.bytecode.ParameterOperands[address]
	return offset, ok
}
...
func (s *GoSynth) Update(patch sointu.Patch, bpm int) error {
	...
	s.bytecode = *bytecode
	// Binding offsets are generation-specific; the host migrates symbolic
	// values after Update rather than allowing stale offsets to leak.
	s.instrumentControl = map[int]float32{}
	for i := range s.voiceControl {
		s.voiceControl[i] = nil
	}
...
func (s *GoSynth) Render(buffer sointu.AudioBuffer, maxtime int) (samples int, renderTime int, renderError error) {
	...
	for renderTime < maxtime && len(buffer) > 0 {
		...
		for voicesRemaining > 0 {
			...
			voiceIndex := len(s.state.voices) - len(voices)
			for i := 0; i < tcount; i++ {
				operandIndex := len(s.bytecode.Operands) - len(operands)
				value := float32(operands[0])
				if controlled, ok := s.instrumentControl[operandIndex]; ok {
					value = controlled
				}
				if controls := s.voiceControl[voiceIndex]; controls != nil {
					if controlled, ok := controls[operandIndex]; ok {
						value = controlled
					}
				}
				params[i] = value/128.0 + unit.ports[i]
				unit.ports[i] = 0
				operands = operands[1:]
			}
			l := len(stack)
