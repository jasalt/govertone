...
type (
	// ParameterAddress identifies a settable/modulatable unit parameter in a patch.
	ParameterAddress struct {
		Instrument int
		Unit       int
		Parameter  string
	}
	...
	Bytecode struct {
		...
		// ParameterOperands maps source patch parameters to their immutable
		// operand offsets. It is used only by the controlled Go VM adapter.
		ParameterOperands map[ParameterAddress]int
	}
	...
	for instrIndex, instr := range patch {
		...
		for unitIndex, unit := range instr.Units {
			...
			case "oscillator":
				...
				b.recordOperands(unit, len(b.Operands))
...
func newBytecodeBuilder(patch sointu.Patch, bpm int) *bytecodeBuilder {
	...
	c := bytecodeBuilder{
		Bytecode:        Bytecode{PolyphonyBitmask: polyphonyBitmask, NumVoices: uint32(patch.NumVoices()), DelayTimes: delayTimesU16, ParameterOperands: map[ParameterAddress]int{}},
		sampleOffsetMap: map[SampleOffset]int{},
		globalAddrs:     map[int]uint16{},
		globalFixups:    map[int]([]int){},
		localAddrs:      map[int]uint16{},
...
func (b *bytecodeBuilder) recordOperands(unit sointu.Unit, offset int) {
	for _, parameter := range sointu.UnitTypes[unit.Type] {
		if !parameter.CanModulate || !parameter.CanSet {
			continue
		}
		b.ParameterOperands[ParameterAddress{Instrument: b.instrumentNo, Unit: b.unitIndex, Parameter: parameter.Name}] = offset
		offset++
	}
}
